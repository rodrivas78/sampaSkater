...
#include "content/tile/city_ro_out_8.h"
// Sprites by GrafxKid (https://opengameart.org/content/super-random-sprites)
#include "content/data_sprt_layer_rod.h"
...
void main()
{
	...
	// Load tiles pattern
	VDP_LoadPattern_GM2(table_Patterns, 94, 0);
	VDP_LoadColor_GM2(table_Colors, 94, 0);
	
	// Initialize scroll module
	u8 sprt = Scroll_Initialize((u16)table_Names);
	...
	SET_SPRITE(sprt+0, 70, 100, 32, COLOR_BLACK);
	SET_SPRITE(sprt+1, 70, 100, 36, COLOR_BLACK);
	SET_SPRITE(sprt+2, 70, 132, 40, COLOR_BLACK);
	SET_SPRITE(sprt+3, 70, 132, 44, COLOR_BLACK);

	SET_SPRITE(sprt+4, 140, 132, 48, COLOR_BLACK);
	VDP_DisableSpritesFrom(sprt+5);
	...
	while (1)
	{
		...
		Scroll_Update();
        //Movimenta o sprite apenas na coordenada X 
		u8 sprtOffset = g_Scroll_OffsetX % 8;
		VDP_SetSpritePositionX(sprt+0, 70 + sprtOffset);
		VDP_SetSpritePositionX(sprt+1, 102 + sprtOffset);
		VDP_SetSpritePositionX(sprt+2, 70 + sprtOffset);
		VDP_SetSpritePositionX(sprt+3, 102 + sprtOffset);

		VDP_SetSpritePositionX(sprt+4, 102 + sprtOffset+50);
		...
		VDP_SetSpritePattern(sprt+3, 44 + shape * 16);

		VDP_SetSpritePattern(sprt+4, 48 + shape * 16);
